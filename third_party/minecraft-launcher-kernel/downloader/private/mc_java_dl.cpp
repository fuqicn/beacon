...
// Fetch Java runtime manifest from all available sources sequentially.
// Sources are tried in order: Mojang -> BMCLAPI (if configured) -> Adoptium.
// Each source has a 5s timeout. We use sequential fetches (not concurrent
// std::threads) because bare std::threads with nested QEventLoop::exec()
// compete with the main GUI event loop and cause UI freezes.
int mc_java_download_manifest(int major_version, const char *mirror, McJavaFileList *list) {
    memset(list, 0, sizeof(*list));

    // Build ordered list of candidate sources.
    ...
    for (auto &src : sources) {
        ...
        mc_info("Java manifest: trying %s ...", src.name);
...
// Legacy concurrent manifest fetch (pre-fix version). Spawns a bare
// std::thread per candidate source; each thread runs mc_http_get which
// embeds a nested QEventLoop::exec() over a thread-local
// QNetworkAccessManager. Those bare threads compete with the main GUI event
// loop over Qt's global (non-thread-safe) event dispatcher, which is the
// historical root cause of the Java-download UI freeze. The current
// mc_java_download_manifest above replaced this with sequential mc_http_get
// calls on the calling thread.
//
// This function is kept verbatim so the manual memory-optimization tool can
// reproduce the exact stall by invoking it on a worker thread and cancelling
// as soon as the file count is known. It is NOT used by the normal download
// path. The freeze ends the moment the manifest returns (file count known).
int mc_java_download_manifest_legacy(int major_version, const char *mirror, McJavaFileList *list) {
    ...
    mc_http_response_free(win_resp);
    if (parseErr.error != QJsonParseError::NoError || !doc.isObject()) {
        mc_error("Failed to parse Java manifest JSON (legacy)");
        return 0;
    }
    QJsonObject root = doc.object();
    ...
    if (rtVal.isUndefined() || !rtVal.isArray()) {
        mc_error("No Java runtime '%s' found in manifest (legacy)", version_key);
        return 0;
    }
    QJsonArray rtArr = rtVal.toArray();
    ...
    QJsonValue rtEntryVal = rtArr.at(0);
    if (rtEntryVal.isUndefined()) {
        mc_error("Empty Java runtime array (legacy)");
        return 0;
    }
    QJsonObject rtEntryObj = rtEntryVal.toObject();
    ...
    QJsonValue manifestVal = rtEntryObj.value("manifest");
    if (manifestVal.isUndefined()) {
        mc_error("No manifest in Java runtime entry (legacy)");
        return 0;
    }
    QJsonObject manifestObj = manifestVal.toObject();
    QString manifestUrl = manifestObj.value("url").toString();
    if (manifestUrl.isNull()) {
        mc_error("No manifest URL in Java runtime entry (legacy)");
        return 0;
    }
    ...
    McHttpResponse *resp = mc_http_get(&client, manifest_url_translated);
    if (!resp || !resp->success || !resp->data) {
        mc_error("Failed to fetch Java runtime file manifest (legacy)");
        if (resp) mc_http_response_free(resp);
        return 0;
    }
    ...
    mc_http_response_free(resp);
    if (fileParseErr.error != QJsonParseError::NoError || !fileDoc.isObject()) {
        mc_error("Failed to parse file manifest JSON (legacy)");
        return 0;
    }
    QJsonObject fileRoot = fileDoc.object();
    ...
    QJsonValue filesVal = fileRoot.value("files");
    if (filesVal.isUndefined() || !filesVal.isObject()) {
        mc_error("No files in Java runtime manifest (legacy)");
        return 0;
    }
    QJsonObject filesObj = filesVal.toObject();
    ...
    if (file_list.empty()) { mc_error("No downloadable files in Java manifest (legacy)"); return 0; }
    ...
    mc_info("Java manifest: %d files for %s (legacy)", list->count, version_key);
    return 1;
}
